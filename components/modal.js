...
export default function Modal({ createNextRound, disabled, saved }) {
  ...
  return (
    <>
      <button onMouseEnter={() => { setShowTip(true) }} onMouseLeave={() => setShowTip(false)} onClick={() => setShowModal(true)} className={`${disabled ? "opacity-25 cursor-default" : "hover:bg-blue-700 cursor-pointer"} flex space-x-2 items-center bg-blue-500 text-white font-bold py-2 px-4 rounded max-w-fit `}>
